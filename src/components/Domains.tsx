...
import { useSiteTexts, useDomaines, useSectionVisibility } from '@/hooks/useSupabaseData';
...
const Domains = () => {
  ...
  const isPageVisible = visibility?.page_domaines ?? true;
              ...
              return (
                <div key={domaine.id} className="card-elegant bg-white group hover:shadow-hover animate-scale-in border-2 hover:border-primary/20" style={{ animationDelay: `${index * 0.1}s` }}>
                  <div className="flex justify-center mb-8">
                    {domaine.icon ? (
                      <div className="text-5xl text-primary group-hover:scale-110 transition-transform duration-300">{domaine.icon}</div>
                    ) : (
                  ...
                  <h3 className="font-heading font-semibold text-xl text-gray-dark mb-6 text-center">{domaine.title}</h3>
                  <p className="text-gray-muted text-center leading-relaxed">{domaine.description}</p>
                </div>
              );
            })}
          </div>
